...
define(function(require,exports,module){
	...
	*/
	...
	window.ArrayPlus = function(obj){
		...
		replaceChildProp:function(propName,targetProp,isDel){
			isDel = isDel||false;
			for (var i = 0; i < this.length; i++) {
				this[i][targetProp] = angular.copy(this[i][propName]);
				if(isDel) delete this[i][propName];
			}
			return this;
		},
		//子元素对象中设置属性
		...
		extractChildArray : function(key,strFlag){
				...
				for (var i = 0; i < this.length;i++) {
					var temp = this[i];
					if(temp[key]) targetArrayList.push(temp[key]);
				}
	...
	$.ajaxSetup({
		error : function(jqXHR, textStatus, errorMsg){
			...
			// }
			if(jqXHR.status === 401){
				mui.alert('您已经过长时间未登陆,请重新登陆','温馨提示',function(){
						window.close();
				});
			}
