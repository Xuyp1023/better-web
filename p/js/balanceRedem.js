	...
	require.async(['p/js/common/dicTool','l/bootstrap/js/bootstrap.min',"l/My97DatePicker/4.8/WdatePicker.js"],function(){
		require.async(['BTDictData'],function(){
				...
				var _personModule = {
					...
					// 初始化余额宝数据
					// 初始化余额宝数据
					initPurchaseFund:function(){

						var routePms = location.hash.substring(1).split('-');
						var fundCode = routePms[0];
						var custNo = routePms[1];
						...
						$.post(common.getRootPath()+'/SaleQuery/queryFundDays',{
							...
							currentData.redemFund = $.extend(true,currentData.redemFund,resultData.data[0]);
							currentData.redemFund.custNo = custNo;
							currentData.redemFund.validShares = routePms[2];
							viewModel.redemFundBind(currentData.redemFund);
							...
							$.post(common.getRootPath()+'/SaleQuery/queryFundLimit',{
								...
								// 计算应该用的限额值
								var maxShare,message;
								if(resultData.data.perMax>routePms[2]){
									maxShare = routePms[2];
									message = '本产品可用份额份额为'+maxShare+'份';
								}else{
									maxShare = resultData.data.perMax;
									message = '本产品最大可赎回份额为'+maxShare+'份';
								}
								validate.validate($('#confirm_amount_form'),{
									elements:[
									{
				...
				//placeholder绑定
				placeholder.placeholder('#input_amount');
				_personModule.initPurchaseFund();
		});
	});
