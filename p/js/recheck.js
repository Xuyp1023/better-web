...
define(function(require,exports,module){
	...
	window.currentData = {
		...
		searchData:{
			GTEoperDate:new Date().getSubDate('MM',3).format('YYYY-MM-DD'),
			LTEoperDate:new Date().format('YYYY-MM-DD'),
			businClass : '01',
			orderStatus :'2'
		},
		crtCountList:[],
	...
	window.viewModel = {
		...
		tradePurchaseListBind : ko.observableArray(currentData.tradePurchaseList),
		...
		searchTicket : function(){
			var id = $('#search_trad_flag option:selected').attr('target');
		...
		showCrtCountDetail : function(data,event){
			location.href = 'recheckInfo.html?param='+data.requestNo+',08,recheck,'+data.id;
		},
		showTradePurchaseDetail : function(data,event){
			location.href = 'recheckInfo.html?param='+data.requestNo+',22,recheck,'+data.id;
		},
