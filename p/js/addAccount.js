		...
		require.async('BTDictData', function () {
			...
			var _personModule = {
				...
				finish_crt_func: function (obj, link) {
					//取出已经存在的机构/公司
					var newFundList = _personModule.filterExistCmp();
					//合并新参数列表
					var newCustInfo = $.extend(currentData.custInfo, { fundList: newFundList });
					//访问账户新增
					$.post(BTPATH.ADD_ACCOUNT_PATH + '?rn=' + Math.random(), common.serializeSingleObject(newCustInfo), function (data) {
						if (data.code == 200) {
							//组装基金公司code以及名称列表
							var targetFundList = [];
				...
				initFundCompList: function () {
					...
					switch (currentData.addTypeInfo.type) {
						case 'fund':
							var SaleAgency = BTDict.SaleAgency;
							SaleAgency.del('ylb');
							data = SaleAgency.toArray('value', 'text');
				...
				initOrgInfo: function (custNo, callback) {
					...
					$.post(BTPATH.RELA_ACCO_INFO_PATH, { 'custNo': custNo, "businClass": currentData.addTypeInfo.type }, function (data) {
						if (data.code === 200) {
							...
							// 经办人快捷操作
							$.post(BTPATH.FIND_SPECIAL_OPERATOR, { 'clerk': "1" }, function (data) {
								if (data.code == 200) {
									var targetElement = $('#SPECIAL_OPERATOR');
									for (var i = 0; i < data.data.length; i++) {
										var tempDic = data.data[i];
										targetElement.append('<option value="' + i + '">' + tempDic.name + '</option>');
									}

									targetElement.change(function () {
										var index = $(this).val();
										triggleChange(data.data[index]);
									});

									function triggleChange(tmpData) {
										currentData.custInfo.contName = tmpData.name;
										currentData.custInfo.contIdentType = tmpData.identType;
										currentData.custInfo.contIdentNo = tmpData.contIdentNo;
										currentData.custInfo.contCertValidDate = tmpData.validDate;
										currentData.custInfo.contMobileNo = tmpData.mobileNo;
										currentData.custInfo.contEmail = tmpData.email;
										currentData.custInfo.contPhone = tmpData.phone;
										currentData.custInfo.contFax = tmpData.faxNo;
										viewModel.regInfoBind(currentData.custInfo);
									}
									triggleChange(data.data[0]);
				...
				reFreshFundCompany: function () {
					var fundCompListInfo;
					switch (currentData.addTypeInfo.type) {
						case 'fund':
							var SaleAgency = BTDict.SaleAgency;
							SaleAgency.del('ylb');
							fundCompListInfo = SaleAgency.toArray('value', 'text');
							// fundCompListInfo = BTDict.SaleAgency.toArray('value', 'text');
							break;
						case 'scf':
							fundCompListInfo = BTDict.ScfAgencyGroup.toArray('value', 'text');
							break;
					}
					...
					fundCompList.find('li').click(function () {
						...
						$.post(BTPATH.SALE_AGENCY_INFO_PATH, { agencyNo: agencyNo }, function (data) {
							if (data.code === 200) {
			...
			$('[mapping="companyNt"]').change(function () {
				currentData.custInfo.companyNt2 = $('[mapping="companyNt"]').find("option:selected").text();
				if (currentData.custInfo.companyNt == '08') {
					viewModel.regInfoBind(currentData.custInfo);
				} else {
					viewModel.regInfoBind(currentData.custInfo);
				}
