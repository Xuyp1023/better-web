exports.installController = function(mainApp,common){
	...
	mainApp.controller('todoFlowController',['$scope','muiSupport','http','$rootScope','$route','cache',function($scope,muiSupport,http,$rootScope,$route,cache){
		/*私有属性区域*/
		$scope.beginPicker = {};
    	$scope.endPicker = {};
      ...
      //   if(console) console.log(items);
		...
		$scope.queryList = function(){
			muiSupport.showBar();
			return http.post(BTPATH.QUERY_WORKFLOW_CURRENT_TASK,$.extend({},$scope.listPage,$scope.searchData,{flag:1})).success(function(data){
				muiSupport.hideBar();
		...
		//打开详情
		$scope.openHandle = function(item){
			cache.put("WORKFLOW_TASK_FLAG", {readonly:'operate'});
			cache.put("currentTask", item);
			window.location.href="flow.html#" + item.workFlowNode.wechatForm; 
		};

		$scope.goDidFlow = function() {
			window.location.href="#/flow/didList";
		}
