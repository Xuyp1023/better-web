...
define(function(require,exports,module){
	...
	exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
		...
		mainApp.controller('supplier.contractAdd',['$scope','http','$rootScope','$route','cache','upload','$location','$window',function($scope,http,$rootScope,$route,cache,upload,$location,$window){
      /*  VM绑定区域  */
      ...
      /*  业务处理绑定区域  */
			...
			$scope.queryCoreCustList = function(){
				return $.post(BTPATH.QUERY_DIC_CORECUSTLIST,{custNo:$scope.info.supplierInfo.custNo},function(data){
					if(common.isCurrentData(data)){
						$scope.$apply(function(){
							$scope.coreList = data.data;
							$scope.info.buyerNo=common.filterArrayFirst(data.data);
						});
					}
				},'json');
			};
			...
			$scope.findSupplierCustInfo = function(){
				$.post(BTPATH.FIND_CONTRACT_LEDGER_CUSTINFO,{custNo:$scope.info.buyerNo},function(data){
					if(common.isCurrentData(data)){
						$scope.$apply(function(){
			...
			$scope.addContract = function(target){
				...
				$scope.info.supplierNo=$scope.info.supplierInfo.custNo+"";
