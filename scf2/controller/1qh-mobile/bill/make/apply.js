...
define(function(require,exports,module){
  ...
  exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
    ...
    mainApp.controller('check.invoiceapply',['$scope','http','$rootScope','$route','cache','commonService',function($scope,http,$rootScope,$route,cache,commonService){
      /*  VM绑定区域  */
    ...
    $scope.statusList = BTDict.commissionInvoiceBusinStatus.toArray('value','name');
    $scope.factorList = [];
    


    //打开普通发票
    $scope.addInfoBox = function(numType){
      
      cache.put("searchData",$scope.searchData);
      cache.put("invoiceType",{type:numType});
       window.location.href = '../scf2/home.html?rn='+new Date().getTime()+'/#/1qh-mobile/applya';
    }
    
    $scope.outbill = function(id){
      cache.put("searchData",$scope.searchData);
      window.location.href = '../scf2/home.html?rn='+new Date().getTime()+'/#/1qh-mobile/outbill/'+id;
      
    }
    $scope.LookdetailInfo = function(invoiceId){

      cache.put("searchData",$scope.searchData);
      window.location.href = '../scf2/home.html?rn='+new Date().getTime()+'/#/1qh-mobile/Invoicedetails/2/'+invoiceId;
    }
      ...
      $scope.$on('$routeChangeSuccess',function(){
         ...
         commonService.queryBaseInfoList(BTPATH.QUERY_DIC_ADDACCCOUNTCOMLIST,{},$scope,'factorList').success(function(){
          ...
          // $scope.searchData.coreCustNo = $scope.searchData.coreCustNo?$scope.searchData.coreCustList.length>0?$scope.searchData.coreCustList[0].value:'';
