...
define(function(require,exports,module){
  ...
  exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
    ...
    mainApp.controller('lead.lead',['$scope','http','$rootScope','$route','cache','commonService','$location','toLoad',function($scope,http,$rootScope,$route,cache,commonService,$location,toLoad){
      /*  VM绑定区域  */
      ...
      $scope.addInfo=function(){
        window.location.href='../byte/home1.html#/qh/commission/import?importDate='
          +new Date().format('YYYY-MM-DD')+'&custNo='+$scope.searchData.custNo;
      } 
      ...
      $scope.delInfo = function(target,data){
        ...
        dialog.confirm('是否确认删除该文件,该操作无法恢复!',function(){
          var $target = $(target);
          http.post(BTPATH.QUERY_COMMISSION_FILE_DELETE,{refNo:data.refNo})
      ...
      var $target=$(target);
        ...
        toLoad.toggleSpin();
        http.post(BTPATH.QUERY_COMMISSION_FILE_RESOLVE/*2*/,{refNo:data.refNo})
          .success(function(data){
              cacheUrl=false;
            //关闭加载状态弹幕
                toLoad.toggleSpin();
