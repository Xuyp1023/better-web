    ...
    mainApp.controller('submit.apply',['$scope','http','$rootScope','$route','cache','commonService','$location','flowService',function($scope,http,$rootScope,$route,cache,commonService,$location,flowService){
     ...
     //资产详情
      ...
      $scope.searchProList = function() {
          // 融资银行
                  commonService.queryBaseInfoList(BTPATH.QUERY_BANK_LIST,{custNo:$scope.info.coreCustNo},$scope,'factorList').success(function(){
                      $scope.$apply(function(){
                           $scope.info.factoryNo=$scope.factorList[0]? $scope.factorList[0].value : '';
                      })
