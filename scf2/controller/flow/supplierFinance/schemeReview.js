...
define(function(require,exports,module){
  ...
  exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
    ...
    mainApp.controller('schemeReview',['$scope','http','$rootScope','$route','cache','commonService','$location','flowService',function($scope,http,$rootScope,$route,cache,commonService,$location,flowService){
      /*  VM绑定区域  */
      ...
      */
      ...
      var validOption = {
            elements: [{
                name: 'info.approvedBalance',
                rules: [{name: 'required'}],
                events: ['blur']
            },{
                name: 'info.approvedPeriod',
                rules: [{name: 'required'}],
                events: ['blur']
            },{
                name: 'info.approvedRatio',
                rules: [{name: 'required'}],
                events: ['blur']
            },{
                name: 'info.info.extensionRatio',
                rules: [{name: 'required'}],
                events: ['blur']
            },{
                name: 'info.overdueRatio',
                rules: [{name: 'required'}],
                events: ['blur']
            }],
      ...
      $scope.approveReview=function(){
        $scope.info.approvedPeriodUnit=$scope.info.approvedPeriodUnit||$scope.periodUnits[0].value;
      ...
      $scope.saveReview=function(target){          
        ...
        //设置校验项 | 校验
        validate.validate($('#approveBox'),validOption);
        var valid = validate.validate($('#approveBox'));
        if(!valid) return;

        http.post(BTPATH.ADD_SCHEME,$scope.info)
