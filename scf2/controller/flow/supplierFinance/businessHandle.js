...
define(function(require,exports,module){
  ...
  exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
    ...
    mainApp.controller('businessHandle',['$scope','http','$rootScope','$route','cache','commonService','$location','flowService',function($scope,http,$rootScope,$route,cache,commonService,$location,flowService){
      /*  VM绑定区域  */
      ...
      */
      ...
      $scope.saveProperty=function(target){          
        var $target = $(target);         
        if($scope.assReg.checkResult==0){
          $scope.assReg.zdwRegCode='';
          $scope.assReg.zdwRegDate='';
          $scope.attachList=[];
        }
        var targetEle=$('#targetEle');        

        http.post(BTPATH.ADD_ASSET_CHECK,$scope.assReg)
           .success(function(data){
            if(data&&(data.code === 200)){
              tipbar.infoTopTipbar('保存成功!',{});                
              $scope.closeRollModal("propertyInfo");
