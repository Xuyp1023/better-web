...
define(function(require,exports,module){
  ...
  exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
    ...
    mainApp.controller('assetmanage.Commercial.invoiceaudit',['$scope','http','$rootScope','$route','cache','commonService',function($scope,http,$rootScope,$route,cache,commonService){
      ...
      $scope.infoBox=function(data){
          ...
          commonService.queryBaseInfoList(BTPATH.QUERY_ATTACHMENT_LIST,{batchNo:data.batchNo},$scope,'uploadList').success(function(){
          // 区分作废还是详情isCancel
          $scope.info=$.extend({},data,{isCheck:'1'});
          $scope.info.isCancel=2;
          $scope.openRollModal('details_box');
        });
      ...
      // 子页面审核按钮的处理
      $scope.doCheck = function(target,data){
        var $target = $(target);        
        http.post(BTPATH.SAVE_AUDIT_INVOICE_RECORD_CORE,{refNo:data.refNo,version:data.version})
