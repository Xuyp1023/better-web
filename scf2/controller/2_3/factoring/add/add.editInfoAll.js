...
define(function(require,exports,module){
	...
	exports.installController = function(mainApp,common,validate,tipbar,loading,comdirect,dialog){
		...
		mainApp.controller('add.editInfoAll',['$scope','http','$rootScope','$route','cache','commonService','$location',function($scope,http,$rootScope,$route,cache,commonService,$location){
      /*  VM绑定区域  */
      ...
      var validOption1 = {
           elements: [/* {
            ...
            errorPlacement: function(error, element) {
                ...
                tipbar.errorLeftTipbar(element,label+error,0,99999,'container');
            }
      };
      //校验配置2
      var validOption2 = {
      ...
      $scope.transform=function(obj){
        ...
        $scope.info=$.extend({},{coreCredict1:'own',isShow:1},cache.get("info1"));               
        $scope.assetDictList=$scope.transform(cache.get("assetDictList"));
        $scope.checkedAssetDictList=$scope.transform(cache.get("checkedAssetDictList"));
        // 操作列表
        commonService.queryBaseInfoList(BTPATH.QUERY_DIC_ADDACCCOUNTCOMLIST,{},$scope,'factorList');

        // 将已选清单列表id提取出来
        var tempArr1=ArrayPlus($scope.checkedAssetDictList).extractChildArray('id',false);
        //将已选清单列表的复选框打钩 
        // angular.forEach($scope.assetDictList,function(row){
        //     angular.forEach($scope.checkedAssetDictList,function(temp){
        //       if(row.id==temp.assestId){
        //         row.checkOne=true;
        //         row.type=temp.assestType;
        //       }
        //     })
        //   });
        for(var i=0,list=$scope.assetDictList;i<list.length;i++){
          for(var j=0,checkedList=$scope.checkedAssetDictList;j<checkedList.length;j++){
            if(checkedList[j].id==list[i].id){
              list[i].checkOne=true;
              list[i].type=checkedList[j].assetType;
              j=checkedList.length;
            }else{
              list[i].type='0';
            }
          }
        ...
        // 提取融资清单的类型为主体资产/辅助材料
        $scope.tempObject=ArrayPlus($scope.assetDictList).extractChildArray('type',false);
        if($scope.tempObject.join('').indexOf(1)!=-1){
          $scope.selected=true;
        }

        $scope.coreList=$scope.transform(cache.get("coreList"));        
        $scope.checkedCoreList=$scope.transform(cache.get("checkedCoreList"));
        // 将已选核心列表id提取出来
        var tempArr2=ArrayPlus($scope.checkedCoreList).extractChildArray('coreCustNo',true);
        angular.forEach($scope.coreList,function(row){
          if(tempArr2.indexOf(row.value)!=-1){
            row.isSelected=true;
          }
        });
         $scope.selectOne();
