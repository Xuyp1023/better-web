...
define(function(require,exports,module){
	...
	mainApp.controller('mainController',['$scope','http','cache',function($scope,http,cache){
		/*VM绑定区域*/
		$scope.statusList = ArrayPlus(BTDict.CustWhitelistStatus.toArray('value','name')).extractChildArrayByindexArray(['1','2']);
		...
		$scope.queryList = function(flag){
			...
			http.post(BTPATH.QUERY_ACCEPT_WHITELIST,$.extend({},$scope.listPage,$scope.searchData))
		...
		$scope.acceptInfo = function(target){
			var $target = $(target);
			http.post(BTPATH.ACCEPT_WHITELIST,{id:$scope.info.id,auditOpinion:$scope.auditOpinion})
				 .success(function(data){
				 	if(common.isCurrentResponse(data)){
				 		$scope.closeRollModal('accept_white_list_box','fast',function(){
				 			dialog.success('白名单受理成功!',3000);
				 		});
				 		$scope.queryList(true);
				 	}else{
		...
		$scope.rejectInfo = function(target){
			var $target = $(target);
			http.post(BTPATH.REJECT_WHITELIST,{id:$scope.info.id,auditOpinion:$scope.auditOpinion})
				 .success(function(data){
				 	if(common.isCurrentResponse(data)){
				 		$scope.closeRollModal('reject_white_list_box','fast',function(){
				 			dialog.success('白名单驳回成功!',3000);
				 		});
				 		$scope.queryList(true);
				 	}else{
				 		tipbar.errorTopTipbar($target,'提交失败,服务器返回:'+data.message,3000,9992);
		...
		*/
		...
		$scope.acceptWhiteList = function(item){
			...
			window.location.href = window.BTServerPath + '/scf2/home.html#/customerRelation/openFactorAccept/'+item.custNo;
		};
		...
		$scope.openInfo = function(item){
			$scope.info = item;
			//从白名单受理-》查看开通保理业务详情
			cache.put("factor_detail_orgin",'whiteListAccept');
			cache.put('relateType',item.relateType);
