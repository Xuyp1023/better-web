  ...
  mainApp.controller('mainController',['$scope','commonService','http',function($scope,commonService,http){
   ...
   //查询催收列表
    $scope.searchList = function(){
        ...
        $scope.reFreshBadDebtList(true);
    };


    //刷新坏账列表数据
    $scope.reFreshBadDebtList = function(flag){
        //是否需要分页信息 1：需要 2：不需要
        ...
        http.post(BTPATH.QUERY_PRESS_MONEY_LIST,$.extend({},$scope.pageConf,$scope.searchData)).success(function(data){
            ...
            if((data!==undefined)&&(data.data!==undefined)&&(data.data!=='')&&(data.code === 200)){
              $scope.$apply(function(){
                  $scope.badDebtList = common.cloneArrayDeep(data.data);
                  if(flag){
...
*/
    ...
    //坏账确认
    $scope.openEnsureBox= function(){
      $scope.openRollModal('bad_debt_ensure_box');
    };
