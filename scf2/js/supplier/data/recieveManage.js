			...
			http.post(BTPATH.EDIT_INFO_RECIEVE,$scope.info)
				 .success(function(data){
				 	if(data&&(data.code === 200)){
						tipbar.infoTopTipbar('修改应收账款成功!',{});
						$scope.closeRollModal("edit_box");
						$scope.queryList(true);

				 	}else{
				 		tipbar.errorTopTipbar($target,'修改应收账款失败,服务器返回:'+data.message,3000,6662);
				 	}
				 });
		};
		...
		*/
		...
		$scope.addInfo = function(target){
			...
			http.post(BTPATH.ADD_INFO_RECIEVE,$scope.info)
				 .success(function(data){
				 	if(data&&(data.code === 200)){
				 		tipbar.infoTopTipbar('添加应收账款成功!',{});
				 		$scope.closeRollModal('add_box');
				 		$scope.queryList(true);
				 	}else{
				 		tipbar.errorTopTipbar($target,'添加应收账款失败,服务器返回:'+data.message,3000,6662);
				 	}
				 });
		};
