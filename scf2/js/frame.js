...
define(function (require, exports, module) {
	...
	window.viewModel = {
		...
		contentIframeLoad: function (data, event) {
			...
			// console.log("frame toHiehht "+toHeight);
			content_iframe.stop().animate({
				height: toHeight + 'px'
			}, 500);
		},
		login: function (data, event) {
			$.post(common.getRootPath() + '/login', { username: 'admin', password: '123456' }, function (data) {
				if (data.code === 200) {
					currentData.userInfo.userName = 'admin';
					currentData.userInfo.status = '成功';
				} else {
					currentData.userInfo.status = '失败';
				}
				viewModel.userInfoBind(currentData.userInfo);
			}, 'json');
		},
		logout: function () {
