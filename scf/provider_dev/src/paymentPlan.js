...
define(function(require,exports,module){
	...
	mainApp.controller('mainController',['$scope','mainService',function($scope,mainService){
		...
		$scope.pageEmitter = {
			...
			endPage : function(){
				$scope.listPage.pageNum=$scope.listPage.pages;
				$scope.queryList(false);
			},
			prevPage : function(){
				$scope.listPage.pageNum--;
				$scope.queryList(false);
			},
			nextPage : function(){
				$scope.listPage.pageNum++;
			...
			skipPage : function(data,event){
				...
				if(isNaN(pageNum)||pageNum.split('.').length>1||Number(pageNum)<1||Number(pageNum)>$scope.listPage.pages){
					$('#fund_list_page [name="skipToPageNum"]').val('');
					tipbar.errorTipbar($(event.target),'请填写正确的页数!',1000);
					return;
				}
				$scope.listPage.pageNum = Number($('#fund_list_page [name="skipToPageNum"]').val()).toFixed(0);
				$('#fund_list_page [name="skipToPageNum"]').val('');
				$scope.queryList(false);
			}
		};
