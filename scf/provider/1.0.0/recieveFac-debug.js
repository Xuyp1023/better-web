...
作者:bhg
*/
define("p/provider/1.0.0/recieveFac-debug", [ "./libs/validate-debug", "./libs/tooltip-debug", "./libs/common-debug", "./libs/dialog-debug", "dialogEx-debug", "./libs/loading-debug", "./libs/commondirect-debug" ], function(require, exports, module) {
    require.async([ "dicTool-debug", "bootstrap-debug", "datePicker-debug" ], function() {
        require.async([ "BTDictData-debug" ], function() {
            ...
            mainApp.controller("testController", [ "$scope", function($scope) {
                ...
                //开启附件下拉帘
                $scope.showUploadList = function(event) {
                    var target = $(event.target || event.srcElement);
                    $("#upload_list_box").height($("body").height()).slideDown();
                };
                //关闭附件下拉帘
                $scope.backForwardUpload = function() {
                    $("#upload_list_box").slideUp();
                };
                //开启商业票据导入
                $scope.showFacUpload = function(event) {
                    var target = $(event.target || event.srcElement);
                    $("#upload_fac_box").height($("#search_info").height() - 52).slideDown();
                };
                //关闭商业票据导入
                $scope.cancelFacUpload = function() {
                    $("#upload_fac_box").slideUp();
                };
                //开启票据附件上传
                $scope.showPlusUpload = function(event) {
                    var target = $(event.target || event.srcElement);
                    $("#upload_plus_box").height($("#fix_operator_info_box").height() - 52).slideDown();
                };
                //关闭票据附件上传
                $scope.cancelPlusUpload = function() {
                    $("#upload_plus_box").slideUp();
                };
                //日期处理
                $scope.dateEmitter = {
